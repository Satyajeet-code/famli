...
After you have the required fields, ask ONCE whether the user wants to override the defaults (6% inflation, 7% expected return). If the user accepts in ANY way without giving numbers ("yes", "ok", "sure", "use defaults", "go with those", "fine", etc.) — pass inflation_rate=0.06 and expected_return=0.07 to the tool. Only use different values if the user explicitly provides numbers. Do not ask twice.
