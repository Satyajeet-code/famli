...
class CalculatorInputsValid:
    """Validated, fully-populated inputs ready to feed the formula."""
    current_age: int
    retirement_age: int
    life_expectancy: int
...
def _resolve(inputs: RetirementInputs) -> CalculatorInputsValid:
    """Apply defaults for any optional input the user hasn't supplied."""
...
def calculate_corpus(inputs: RetirementInputs) -> CorpusResult:
    ...
    """
    valid = _resolve(inputs)

    if valid.retirement_age <= valid.current_age:
        raise ValueError("retirement_age must be greater than current_age")
    if valid.life_expectancy <= valid.retirement_age:
        raise ValueError("life_expectancy must be greater than retirement_age")

    n_years_to_retire = valid.retirement_age - valid.current_age
    retirement_period = valid.life_expectancy - valid.retirement_age

    current_annual_expense = valid.monthly_expense * Decimal(12)

    # Step 1: Future Annual Expense at retirement
    inflation_factor = (Decimal(1) + valid.inflation_rate) ** n_years_to_retire
    future_annual_expense = current_annual_expense * inflation_factor

    # Step 2: Real Rate of Return
    real_rate = (
        (Decimal(1) + valid.expected_return) / (Decimal(1) + valid.inflation_rate)
    ) - Decimal(1)
    ...
    return CorpusResult(
        ...
        assumptions_applied=valid.assumptions_applied,
    )
